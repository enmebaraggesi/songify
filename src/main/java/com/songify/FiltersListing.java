...
@Component
@AllArgsConstructor
public class FiltersListing implements CommandLineRunner {
    
    private final ApplicationContext applicationContext;
    
    @Override
    public void run(final String... args) {
        // Lists all registered Filter beans
        Map<String, Filter> filters = applicationContext.getBeansOfType(Filter.class);
        filters.forEach(
                (name, filter) -> System.out.println("Filter name: " + name + "; Filter class: " + filter.getClass().getName()));
        ...
        filterRegistrationBeans.forEach(
                (name, filter) -> System.out.println("FilterRegistration name: " + name + "; FilterRegistration class: " + filter.getClass().getName()));
        
        // Filter all Spring Security filters
        if (applicationContext.containsBean("springSecurityFilterChain")) {
            ...
            for (SecurityFilterChain securityFilterChain : filterChainProxy.getFilterChains()) {
                for (Filter filter : securityFilterChain.getFilters()) {
                    System.out.println("Spring Security Filter: " + filter.getClass().getName());
                }
            }
        }
    }
